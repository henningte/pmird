...
read_opus_bin_univ <- function(file_path, extract = c("spc"),
  ...
  if(!requireNamespace(package = "hexView")) {
    stop("Package 'hexView' (>= 0.3-4) required. Install this package first.")
  }
  if(!requireNamespace(package = "foreach")) {
    stop("Package 'foreach' (>= 1.5.0) required. Install this package first.")
  }
  if(!requireNamespace(package = "data.table")) {
    stop("Package 'data.table' (>= 1.12.8) required. Install this package first.")
  }
  ...
  try({
    ...
    return_spc_param <- function(end_spc, spc_param_list) {
      ...
      param_min <- foreach::foreach(i = 1:length(spc_param_list),
                                    .final = function(i) stats::setNames(i, names(spc_param_list))) %do% {
    ...
    # ---mod:
    if(length(end_spc) > length(NPT_spc)) {
      end_spc <- end_spc[seq_along(NPT_spc)]
    }
    ...
    names(wavenumbers) <- spc_type[seq_along(wavenumbers)] #---mod
    ...
    names(CSF) <- names(spc)[seq_along(CSF)] # ---mod
    if (any(unlist(CSF) != 1)) {
    ...
    if(length(wavenumbers) < length(spc)) {
      for(i in seq_len(length(spc) - length(wavenumbers))) {
        wavenumbers <- c(wavenumbers, wavenumbers[length(wavenumbers)])
      }
    } # ---mod
    spc_m <- foreach::foreach(i = 1:length(spc_m),
                              .final = function(i) stats::setNames(i, names(spc_m))) %do% {
    ...
    metadata <- tibble::tibble(
      ...
      temp_scanner_sm = if(length(TSC_all) == 0){ # ---mod
        NA
      } else {
        TSC_all[[length(TSC_all)]]}
      , # select last element
      ...
      temp_scanner_rf = ifelse(length(TSC_all) <= 1, NA, TSC_all[[1]]), #---mod
      # Relative humidity
      hum_rel_sm = ifelse(length(HUM_rel) == 0, NA, HUM_rel[[length(HUM_rel)]]), # sample measurement ---mod
      hum_rel_rf = ifelse(length(HUM_rel) <= 1, NA, HUM_rel[[1]]), # reference #---mod
      # measurement
    ...
    if(any(is.na(names(spc_m)))) {
      out$spc_corr <- spc_m[which(is.na(names(spc_m)))]
    }
...
read_opus_univ <- function(fnames,
  ...
  if (parallel == TRUE) {
    foreach::foreach(i = 1:length(fnames),
                     ...
                     .final = function(i) stats::setNames(i, sub(".+/(.+)", "\\1", fnames))) %dopar% {
                       try(
